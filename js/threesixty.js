...
$(document).ready(function () {
	...
	*/
	function loadImage() {
		...
		var imageName = "img/360_2/" + (loadedImages + 1) + ".jpg";
		/*
	...
	*/
	function showThreesixty () {
		...
		// ...so when the animation renders, it will initially take 4 complete spins (84x4=720, 84x2=168).
		refresh();

        if(localStorage["strg_mark"]=='marker6' || localStorage["strg_mark"]=='marker3') {
            endFrame = -47;
	...
	*/
	function hidePreviousFrame() {
		/*
			...
			It calls the "getNormalizedCurrentFrame" method to translate the "currentFrame" value to the "totalFrames" range (1-84 by default).
		*/
		frames[getNormalizedCurrentFrame()].removeClass("current-image").addClass("previous-image").parents('li').removeClass("current-li");
	};
	...
	*/
	function showCurrentFrame() {
		/*
			...
			It calls the "getNormalizedCurrentFrame" method to translate the "currentFrame" value to the "totalFrames" range (1-84 by default).
		*/
		frames[getNormalizedCurrentFrame()].removeClass("previous-image").addClass("current-image").parents('li').addClass("current-li");
	};
	...
	*/
    	/* My */
	function zoomIt() {
		// TODO: disable or not user interaction during animation
		// 1) Go to nearest "init" frame
		// calculate
		var gotoFrame = currentFrame/71;
		gotoFrame = Math.round(gotoFrame);
		gotoFrame = gotoFrame * 71;
		endFrame = gotoFrame;
		// 2) Zoom
		// 3) Apply
		refresh();
	...
	function trackPointer(event) {
        ...
        //console.log(Math.abs(currentFrame));

        //$('.wrap_markdesc .mrk').removeClass('active');

        // desc
        //console.log($('.current-li').index());

        // curent slide
        ceurent_id = $('.current-li').index();
        console.log(ceurent_id);

        //$('.marker1').css({'right': ceurent_id*9});
        //$('.marker2').css({'right': ceurent_id*20});
        $('.arow360').css({'display':'none'});

        // group
        if(ceurent_id <= 10 && ceurent_id <= 70) {
        ...
        if(ceurent_id >= 37 && ceurent_id <= 54) {
            $('.group2').css({'display':'block'});
        }
        else {
    ...
    // description
    //audio_file = document.getElementById('player');
    mark = $('.mark');
    btext = $('.text');

    mark.on("tapone", function() {
        if($(this).hasClass('pl_audio')) {
            //audio_file.play();
    ...
    btext.on("tapone", function() {
        if($(this).hasClass('st_audio')) {
            //audio_file.pause();
            localStorage.clear();
